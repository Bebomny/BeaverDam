...
export const POST: RequestHandler = async ({ request, locals, fetch }) => {
    ...
    if(!seriesId) {
        throw error(400, 'Missing seriesId')
    }

    try {
        const userEmail = locals.userEmail || 'unknown@mail'

        const response = await fetch(`${BACKEND_URL}/api/watchpost/series/${seriesId}/metadata/remove`, {
            method: 'POST',
            headers: {
        ...
        if (!response.ok) {
            throw error(response.status, 'Backend failed to remove series metadata');
        }

        return json({success: true});
    } catch (err) {
        console.error("BFF error on metadata removal:", err);
